    ...
    public class PendulumManager : MonoBehaviour
    {
        ...
        private void Update() {
            if (GameManager.state == GameState.PAUSED) {
                ...
                if (isEdit) {
                    // When the user presses the "Tab" key (during Edit mode), we go through and select the next pendulum in the list
                    if (Input.GetKeyDown(KeyCode.Tab)) {
                        // If the "Shift" key is held, we go backwards instead
                        if (Input.GetKey(KeyCode.LeftShift)) { 
                            selectedDoublePendulum = GetPrevious();
                        } else { 
                            selectedDoublePendulum = GetNext(); 
                        }
                        // We update the tab display
                        UI.UpdateTabs();
                    }

                    // If the user presses the "DEL" (aka. "SUPPR" on french keyboards) key while in EDIT mode, we toggle all the remove buttons (hidden behind each tab)
                    if (Input.GetKeyDown(KeyCode.Delete)) { UI.ToggleRemoveButtons(); }
                    // We show/hide remove buttons depending on the toggle value
                    if (UI.isRemoveBtnActive) { UI.ShowRemoveButtons(); }
                    else { UI.HideRemoveButtons(); }
                }
            }
